...
static string _DER(const JValue &data){
	...
	if(data.isBool()){
		...
		for(size_t i = 0; i < arrKeys.size(); i++){
			...
			strDict += strKey + strVal;
		}

		strOutput.append(1, 0x31);
		_DERLength(strOutput, strDict.size());
		strOutput += strDict;
	}else if(data.isFloat() || data.isDate() || data.isData()){
		assert(false);
	}else{
...
bool SlotBuildRequirements(const string &strBundleID, const string &strSubjectCN, string &strOutput){
	...
	uint8_t pack3[] = {0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0x0f, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0x0b, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0a, 0x73, 0x75, 0x62, 0x6a, 0x65, 0x63, 0x74, 0x2e, 0x43, 0x4e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01};
	uint32_t uSubjectCNLength = (uint32_t)strSubjectCN.size();

	//string strPaddedSubjectID
	uint8_t pack4[] = {0x00, 0x00, 0x00, 0x0e, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x0a, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x63, 0x64, 0x06, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

	uLength2 += sizeof(magic2) + sizeof(uLength2) + sizeof(pack2) + sizeof(uBundldIDLength) + strPaddedBundleID.size() + sizeof(pack3) + sizeof(uSubjectCNLength) + strPaddedSubjectID.size() + sizeof(pack4);
	uLength1 += sizeof(magic1) + sizeof(uLength1) + sizeof(pack1) + uLength2;
