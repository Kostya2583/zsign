...
const char *appleDevCACert = "-----BEGIN CERTIFICATE-----\n"
...
//    CMS_add1_signer(cms, NULL, NULL, EVP_sha1(), nFlags);
	...
	X509_ATTRIBUTE_set1_data(attr, V_ASN1_SEQUENCE, type_256->value.asn1_string->data, type_256->value.asn1_string->length);
...
bool GetCertInfo(X509 *cert, JValue &jvCertInfo){
	...
	jvCertInfo["Validity"]["NotBefore"] = ASN1_TIMEtoString(OPENSSL_VERSION_NUMBER < 0x10100000L ? X509_get_notBefore(cert) : X509_get0_notBefore(cert));
	jvCertInfo["Validity"]["NotAfter"] = ASN1_TIMEtoString(OPENSSL_VERSION_NUMBER < 0x10100000L ? X509_get_notAfter(cert) : X509_get0_notAfter(cert));
