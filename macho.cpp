...
bool ZMachO::ReallocCodeSignSpace(){
	...
	for(size_t i = 0; i < m_arrArchOes.size(); i++){
		ZLog::PrintV("Realloc CodeSignature Space %lu/%lu\n", i + 1, m_arrArchOes.size());
		...
		ZLog::PrintV("Realloc CodeSignature Space %lu/%lu: %u\n", i + 1, m_arrArchOes.size(), uNewLength);
		if(uNewLength <= 0){
			ZLog::Error("Failed!\n");
			return false;
		}
	...
	ZLog::Warn("Reallocating MachO File\n");
	if(1 == m_arrArchOes.size()){
		ZLog::WarnV("Closing Old MachO File: %s\n", m_strFile.c_str());
		CloseFile();
		ZLog::WarnV("Removing Old MachO File: %s\n", m_strFile.c_str());
		RemoveFile(m_strFile.c_str());
		ZLog::WarnV("Renaming New MachO File: %s\n", m_strFile.c_str());
		...
		ZLog::WarnV("Realloc Fat MachO File\n");
		...
		if(arrArches.size() != m_arrArchOes.size()){
			ZLog::Error("Fat Arch Count Not Match!\n");
			return false;
		}
